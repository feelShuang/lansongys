...
#warning lineColor
    _line.backgroundColor = [UIColor colorWithRed:83 / 255.0 green:181 / 255.0 blue:191 / 255.0 alpha:1];
...
#warning buttonTitleColor
        
        if (index == 0) {
            [button setTitleColor:[UIColor colorWithRed:83 / 255.0 green:181 / 255.0 blue:191 / 255.0 alpha:1] forState:UIControlStateNormal];
        }else{
        ...
        UIButton *badgeIamge = [UIButton buttonWithType:UIButtonTypeCustom];
        badgeIamge.frame = CGRectMake(CGRectGetWidth(button.frame) - ITEM_HEIGHT / 2, 0, ITEM_HEIGHT / 2, ITEM_HEIGHT / 2);
        [badgeIamge setBackgroundImage:[UIImage imageNamed:@"new_dot"] forState:UIControlStateNormal];
        badgeIamge.hidden = YES;
        badgeIamge.selected = NO;
        badgeIamge.tag = 2000;
        [button addSubview:badgeIamge];
    }
...
- (void)setBadgeValue:(NSArray *)valueArray {
    
    for (int i = 0; i <_items.count; i ++) {
        if ([valueArray[i] isEqualToString:@"0"]) {

        } else if ([valueArray[i] integerValue] > 99) {
            [_items[i] viewWithTag:2000].hidden = NO;
            [((UIButton *)[_items[i] viewWithTag:2000]) setTitle:@"99+" forState:UIControlStateNormal];
        } else {
            [_items[i] viewWithTag:2000].hidden = NO;
            [((UIButton *)[_items[i] viewWithTag:2000]) setTitle:valueArray[i] forState:UIControlStateNormal];
        }
    }
}
