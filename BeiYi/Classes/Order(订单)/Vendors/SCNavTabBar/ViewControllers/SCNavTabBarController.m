...
@interface SCNavTabBarController () <UIScrollViewDelegate, SCNavTabBarDelegate>
{
    ...
    NSMutableArray  *_orderStatistic;   // 订单统计
...
- (void)initConfig
{
    ...
    // 利用tag值将要显示的订单数量传递过来
    _orderStatistic = [[NSMutableArray alloc] initWithCapacity:_subViewControllers.count];
    for (UIViewController *viewController in _subViewControllers)
    {
        [_titles addObject:viewController.title];
//        [_orderStatistic addObject:[NSString stringWithFormat:@"%ld",[viewController.view.tag - 1000]];
    }
...
//    _navTabBar.backgroundColor = _navTabBarColor;
    ...
    _navTabBar.itemOrderStatistic = _orderStatistic;
...
#warning 更改子视图控制器View的frame
        viewController.view.frame = CGRectMake(idx * SCREEN_WIDTH, DOT_COORDINATE, SCREEN_WIDTH, _mainView.frame.size.height - 44);
...
#pragma mark - 设置badgeValue
- (void)setBadgeValueWithValueArray:(NSArray *)valueArray {
    
    [_navTabBar setBadgeValue:valueArray];
}
