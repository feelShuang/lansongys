//
//  LSBrokerSelectDoctorViewController.m
//  BeiYi
//
...
@interface LSBrokerSelectDoctorViewController ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate>
...
@implementation LSBrokerSelectDoctorViewController
...
- (void)viewDidLoad {
    ...
    // 注册cell
    [self.docTableView registerNib:[UINib nibWithNibName:NSStringFromClass([LSBrokerDoctorTableViewCell class]) bundle:nil] forCellReuseIdentifier:NSStringFromClass([LSBrokerDoctorTableViewCell class])];
    ...
    // 4.加载科室列表
    [self loadHTTPForDepartWithHosID:self.hospital_id];
}
...
- (void)setUI {
    ...
    [rightBtn addTarget:self action:@selector(brokeSearchDoctorData) forControlEvents:UIControlEventTouchUpInside];
...
- (void)brokeSearchDoctorData {
    ...
    [_searchbar mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        // 中心点对齐
        make.center.mas_equalTo(_searchBgView);
        // 大小
        make.size.mas_equalTo(CGSizeMake(SCREEN_WIDTH - 60, CGRectGetHeight(_searchBgView.frame) - 10));
    }];
...
- (void)addDoctorList {
    ...
    self.docTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 40 + 64, SCREEN_WIDTH, SCREEN_HEIGHT - 104) style:UITableViewStylePlain];
    // 设置代理
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == self.hospitalTableView) { // 医院
        ...
        if (indexPath.row < self.doctors.count) {
            cell.manage_doctor = self.doctors[indexPath.row];
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == _hospitalTableView) { // 医院
        ...
        if (myAccount) {
            ...
            // 登陆后查看医生详情
            LSBrokerDoctorDetailViewController *doctorDetailVC = [LSBrokerDoctorDetailViewController new];
...
- (void)addHeaderRefreshWithHospital_id:(NSString *)hospital_id department_id:(NSString *)department_id sort:(NSString *)sort order_type:(NSString *)order_type keyword:(NSString *)keyword {
    ...
    __weak typeof(self) weakSelf = self;
    self.docTableView.mj_header = [LSRefreshGifHeader headerWithRefreshingBlock:^{
        // 1.清除所有数据
        ...
        [weakSelf.docTableView.mj_header endRefreshing];
    }];
}
...
- (void)addFooterRefreshWithHospital_id:(NSString *)hospital_id department_id:(NSString *)department_id sort:(NSString *)sort order_type:(NSString *)order_type keyword:(NSString *)keyword {
    ...
    __weak typeof(self) weakSelf = self;
    self.docTableView.mj_footer = [MJRefreshAutoNormalFooter footerWithRefreshingBlock:^{
        // 1.加载网络请求
        ...
        [weakSelf.docTableView.mj_footer endRefreshing];
    }];
    ...
    [ZZHTTPTool post:urlStr params:params success:^(NSDictionary *responseObj) {
        ...
        if ([responseObj[@"code"] isEqualToString:@"0000"]) {// 数据加载成功
            ...
            if (arr.count < 20) { // 请求下来的数据少于20，表示已经取完。page增加就没有添加的数据了
                ...
                [self.docTableView.mj_footer endRefreshingWithNoMoreData];
            }
