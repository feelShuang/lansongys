//
//  LSConsultationViewController.m
//  BeiYi
//
...
@interface LSConsultationViewController ()<LSManagePatientDelegate,LSTimeSelectDelegate,UITextFieldDelegate>
...
/** 会诊地点 */
@property (weak, nonatomic) IBOutlet UITextField *consulationAddressTextField;
/** 医院名称 */
...
@implementation LSConsultationViewController
...
- (void)textFieldDidBeginEditing:(UITextField *)textField {
    ...
    if (textField == _consulationAddressTextField) {
...
- (void)getPrice {
    ...
    ZZLog(@">_> Look>>>>>>%d,%s,\nparams = %@<<<<<<",__LINE__,__func__,params);
...
- (IBAction)textFieldReturnEdit:(UITextField *)sender {
...
- (IBAction)appointCommit:(UIButton *)sender {
    ...
    if (self.appointTimeLabel.text.length == 0) {
        [MBProgressHUD showError:@"请选择会诊时间"];
        return;
    } else if (self.visitPersonNameLabel.text.length == 0) {
        [MBProgressHUD showError:@"请选择就诊人"];
        return;
    } else if (self.consulationAddressTextField.text.length == 0) {
...
- (void)loadHttpRequest {
    ...
    // 2. 准备请求接口
    NSString *urlStr = [NSString stringWithFormat:@"%@/uc/order/create_consultation",BASEURL];;
    // 2.1 创建请求体
    NSDictionary *params = [NSDictionary dictionary];
    
    params = [self setConsulationRequestBody];
...
#pragma mark - 设置会诊服务的请求体
- (NSDictionary *)setConsulationRequestBody {
    ...
    [params setObject:self.consulationAddressTextField.text forKey:@"visit_address"]; // 12.就诊地址
